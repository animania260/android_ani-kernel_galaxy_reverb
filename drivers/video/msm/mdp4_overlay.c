...
void mdp4_mixer_blend_setup(int mixer)
{
	...
	for (i = MDP4_MIXER_STAGE0; i < MDP4_MIXER_STAGE_MAX; i++) {
		...
		if (mdp_rev >= MDP_REV_42 || ctrl->panel_mode & MDP4_PANEL_MDDI || ctrl->panel_mode & MDP4_PANEL_DSI_CMD || (mdp_rev == MDP_REV_40 && (ctrl->panel_mode & MDP4_PANEL_LCDC) && perf_req->use_ov0_blt))
			outpdw(overlay_base + off + 0x104, blend->op);
