...
static void change_dvfs_lock(struct work_struct *work)
{
	...
	ret = set_freq_limit(DVFS_TOUCH_ID, 486000);
...
#if defined(ESD_DEBUG) || defined(TSK_FACTORY)
...
#if defined(ESD_DEBUG) || defined(TSK_FACTORY)
static u32 get_raw_data_two(struct mms_ts_info *info, u16 rx_idx, u16 tx_idx,
		u8 cmd)
{
	u8 w_buf[6];
	u8 read_buffer[2];
...
static void get_chip_name(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "%s", "MMS128");
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
	info->cmd_state = 2;
	dev_info(&info->client->dev, "%s: %s(%d)\n", __func__,
			buff, strnlen(buff, sizeof(buff)));
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = get_raw_data_two(info, 0, 0, VSC_RAW_TK);
	if (ret < 0)
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = get_raw_data_two(info, 0, 1, VSC_RAW_TK);
	if (ret < 0)
		dev_err(&client->dev, "%s: fail to read (%d)\n", __func__, ret);
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = get_raw_data_two(info, 0, 2, VSC_RAW_TK);
	if (ret < 0)
		dev_err(&client->dev, "%s: fail to read (%d)\n", __func__, ret);
...
static DEVICE_ATTR(touchkey_menu, S_IRUGO, tkey_rawcounter_show0, NULL) ;
static DEVICE_ATTR(touchkey_home, S_IRUGO, tkey_rawcounter_show1, NULL) ;
static DEVICE_ATTR(touchkey_back, S_IRUGO, tkey_rawcounter_show2, NULL) ;
