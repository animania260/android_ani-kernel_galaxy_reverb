...
{ KEY_HOMEPAGE, TOUCH_MENU, TOUCH_BACK, TOUCH_SEARCH, };
...
void mcsdl_vdd_off(void)
{
	...
	regulator_disable(regulator_touchio);
...
static void melfas_read_version(void)
{
	...
	if (0 == melfas_i2c_read(melfas_ts->client, MCSTS_MODULE_VER_REG, buf, 2))
	{
		...
		printk(KERN_ERR "%s :HW Ver : 0x%02x, FW Ver : 0x%02x\n"
	...
	else
	{
		melfas_ts->hw_rev = 0;
		melfas_ts->fw_ver = 0;
		printk(KERN_ERR "%s : Can't find HW Ver, FW ver!\n", __func__);
	}
}
...
static ssize_t tsp_test_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	touch_screen.driver = &melfas_test_mode;
	touch_screen.tsp_info.driver = &(touch_screen.driver->ts_info);
...
static ssize_t tsp_test_reference_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
...
static ssize_t tsp_test_inspection_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	if(strncmp(buf, "01", 2) == 0)
...
static ssize_t tsp_test_delta_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	int i ;
	touch_screen.driver = &melfas_test_mode;
	touch_screen.tsp_info.driver = &(touch_screen.driver->ts_info);
...
static ssize_t tsp_test_sleep_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
...
static ssize_t tsp_test_wakeup_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
...
void set_tsp_noise_filter_reg(void)
{
#if 0
...
static void melfas_work_func(void)
{
	...
	if (ret < 0)
	{
		printk("KERN_ERR [TSP][MMS128][%s] i2c failed : %d\n", __func__, ret);
		tsp_reset();
		return ;
	}
	else
	...
	if (read_num > 0)
	{
		...
		else
		{
			...
			if (buf[0] == 0x0f)
			{
				printk("KERN_ERR [TSP][MMS128][%s] ESD defense!!  : %d\n", __func__, fingerID);
				tsp_reset();
				return;
			}
			for (i = 0; i < read_num; i = i + 6)
			{
				...
				if (touchType == 1)	//Screen
				{
					...
					{
						printk("KERN_ERR [TSP][MMS128][%s] fingerID : %d\n", __func__, fingerID);
						tsp_reset();
						return ;
					}
					...
					printk(KERN_ERR " x: %d y: %d width: %d z: %d",melfas_ts->info[fingerID].x,melfas_ts->info[fingerID].y,melfas_ts->info[fingerID].width,melfas_ts->info[fingerID].z);
				}
#ifdef __TOUCH_KEY__
			...
			if (touched_src)
			{
				for (j = 0; j < TS_MAX_TOUCH; j++) {
					...
					if (melfas_ts->info[j].z != 0) {
						input_report_abs(melfas_ts->input_dev,
								ABS_MT_TRACKING_ID, j);
						input_report_abs(melfas_ts->input_dev,
								ABS_MT_POSITION_X,
								melfas_ts->info[j].x);
						input_report_abs(melfas_ts->input_dev,
	...
	{
		printk(KERN_ERR "[TSP][MMS128][%s] : read_num=%d\n",__func__, read_num);
	}
...
int melfas_ts_probe(struct platform_device *pdev)
{
	...
	mdelay(300);
	mcsdl_vdd_on();
	debugprintk(5,"[TOUCH] Firmware doesn't update. \n");
	...
	set_bit(KEY_HOMEPAGE, melfas_ts->input_dev->keybit);
	...
	//t_bit(BTN_TOUCH, melfas_ts->input_dev->keybit);
...
int melfas_ts_remove(struct platform_device *dev)
{
#ifdef CONFIG_HAS_EARLYSUSPEND
